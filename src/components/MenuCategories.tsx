...
export const MenuCategories: React.FC<MenuCategoriesProps> = ({
  ...
  return (
    <motion.section 
      ...
      className="relative -mt-6 sm:-mt-8 z-20 max-w-6xl mx-auto px-4 sm:px-6"
    >
      <div className="bg-[#fdfbf7] rounded-3xl p-6 sm:p-8 shadow-xl border border-[#e6decf]">
        ...
        {/* Circular Categories Grid matching Screenshot 1 with Staggered Entrance */}
        <div className="flex items-center justify-start md:justify-center gap-4 sm:gap-6 md:gap-8 overflow-x-auto pb-3 pt-1 scrollbar-thin scrollbar-thumb-amber-200">
            ...
            <div
              className={`w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center border-2 transition-all p-1 shadow-md ${
                selectedCategory === null
                  ? 'border-[#d96b0c] ring-4 ring-[#d96b0c]/20 shadow-amber-300'
                  : 'border-stone-300 bg-stone-100 group-hover:border-amber-400'
              }`}
            >
              <div className="w-full h-full rounded-full bg-gradient-to-tr from-[#d96b0c] to-amber-500 flex items-center justify-center text-white font-bold text-xs sm:text-sm shadow-inner">
                Tất cả
              </div>
            </div>
            <span
              className={`text-xs sm:text-sm font-medium mt-2 transition-colors whitespace-nowrap ${
                selectedCategory === null ? 'text-[#d96b0c] font-bold' : 'text-stone-700 group-hover:text-[#d96b0c]'
              }`}
            >
              Xem tất cả
            </span>
          ...
          {CATEGORIES.map((cat, idx) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-xs sm:text-sm font-medium mt-2 transition-colors whitespace-nowrap ${
                    isSelected ? 'text-[#d96b0c] font-bold' : 'text-stone-800 group-hover:text-[#d96b0c]'
                  }`}
                >
                  {cat.name}
                </span>
