...
export const SearchModal: React.FC<SearchModalProps> = ({
  ...
  const filteredDishes = useMemo(() => {
    ...
    return dishes.filter((d) => {
      const name = getDishName(d).toLowerCase();
      const desc = (getDishDesc(d) || '').toLowerCase();
      const rawName = d.name.toLowerCase();
      const rawDesc = (d.description || '').toLowerCase();
      return (
        name.includes(q) ||
        desc.includes(q) ||
        rawName.includes(q) ||
  ...
  }, [query, dishes, getDishName, getDishDesc]);
      ...
      <div className="w-full max-w-2xl bg-white rounded-3xl overflow-hidden shadow-2xl border border-stone-200 animate-fadeIn flex flex-col max-h-[80vh]">
        ...
        <div className="p-4 sm:p-5 border-b-2 border-amber-400/40 flex items-center gap-3 bg-gradient-to-r from-[#7f1d1d] via-[#991b1b] to-[#6b1414] text-amber-100">
          ...
          {query && (
            <button
              onClick={() => setQuery('')}
              className="text-amber-200 hover:text-white text-xs px-2 py-1 font-serif cursor-pointer"
            >
              {t('search.clear')}
            </button>
          )}
        ...
        <div className="overflow-y-auto p-4 sm:p-5 space-y-3 flex-1 bg-[#fffdfa]">
          <div className="text-xs font-serif font-bold uppercase tracking-wider text-amber-900 mb-2 flex items-center gap-2">
            <span className="seal-stamp text-[9px] py-0.5 px-1.5">{t('search.seal')}</span>
            <span>
              {query.trim()
                ? t('search.results_title').replace('{count}', filteredDishes.length.toString())
                : t('search.suggested_title')}
            </span>
          </div>
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">{t('search.order_btn')}</span>
                  </button>
                </div>
              </div>
            );
          ...
          {filteredDishes.length === 0 && (
            <div className="text-center py-10 text-stone-500 text-xs sm:text-sm font-literary">
              {t('search.empty').replace('{query}', query)}
            </div>
          )}
        </div>
