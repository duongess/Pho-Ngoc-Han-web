...
import { MapPin, Phone, Mail, Globe, Facebook, Youtube, GraduationCap } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 pb-12 border-b border-amber-900/30">
          ...
          <div className="lg:col-span-4 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <div className="flex items-center gap-2">
                  ...
                  <span className="seal-stamp text-[9px] py-0.5 px-1.5 hidden sm:inline-block">
                    {lang === 'en' ? 'AUTHENTIC' : 'GIA TRUYỀN'}
                  </span>
                </div>
                <span className="text-xs font-calligraphy text-amber-400/90 tracking-widest text-sm">
                  {lang === 'en' ? 'Subtle & Pure • Crafted with Heart' : 'Đượm vị thanh tao • Gói trọn tình cô giáo'}
                </span>
              </div>
            </div>
            ...
            <p className="text-xs text-stone-400 leading-relaxed pt-2 font-literary">
              {lang === 'en'
                ? 'Rooted in passion for Vietnamese cuisine and dedication, Phở Ngọc Hân preserves the 18-hour simmered bone broth, offering the authentic, delicate heritage flavor of ancient Hanoi.'
            ...
            <div className="flex items-center gap-2 text-xs text-amber-300 font-serif bg-amber-950/40 p-2.5 rounded-xl border border-amber-900/50">
              ...
              <span>
                {lang === 'en'
                  ? 'Beloved culinary meeting spot next to Hanoi University of Civil Engineering (NUCE)'
                  : 'Góc hẹn thân thương kề bên cổng Trường Đại học Xây Dựng Hà Nội'}
              </span>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-3.5">
            ...
            <ul className="space-y-3 text-xs text-stone-300 font-literary">
              <li className="flex items-start gap-2.5">
                ...
                <span>
                  {lang === 'en'
                    ? '55 Giai Phong, Hai Ba Trung, Hanoi (Gate of Civil Engineering University)'
                    : 'Số 55 Giải Phóng, Hai Bà Trưng, Hà Nội (Cạnh cổng ĐH Xây Dựng)'}
                </span>
              </li>
              <li className="flex items-center gap-2.5">
                <Phone className="w-4 h-4 text-amber-400 shrink-0" />
                <div>
                  <span>{t('footer.phone_label')} <strong>0988 567 899</strong></span>
                  <br />
                  <span className="text-amber-300/80">{t('footer.hotline_label')} <strong className="text-amber-200">1900 9077</strong></span>
                </div>
              </li>
              <li className="flex items-center gap-2.5">
          ...
          <div className="lg:col-span-2 space-y-3.5">
            <h4 className="text-sm sm:text-base font-serif font-bold text-amber-300 uppercase tracking-widest flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-400" />
              {t('footer.col_cuisine')}
            </h4>
            <ul className="space-y-2 text-xs text-stone-400 font-literary">
              <li>
                <a href="#thuc-don" className="hover:text-amber-300 transition">{t('footer.link_beef')}</a>
              </li>
              <li>
                <a href="#thuc-don" className="hover:text-amber-300 transition">{t('footer.link_rolls')}</a>
              </li>
              <li>
                <a href="#he-thong-cua-hang" className="hover:text-amber-300 transition">{t('footer.link_stores')}</a>
              </li>
              <li>
                <a href="#uu-dai" className="hover:text-amber-300 transition">{t('footer.link_promo')}</a>
              </li>
              <li>
                <a href="#dat-mon" className="hover:text-amber-300 transition">{t('footer.link_delivery')}</a>
              </li>
            </ul>
          </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="bg-[#fffdfa] text-stone-900 rounded-2xl p-3.5 shadow-xl border-2 border-amber-400/60">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-stone-500 font-literary">
                    {lang === 'en' ? 'Loved by 45,000+ patrons' : 'Hơn 45.000 thực khách mến mộ'}
                  </span>
                </div>
              </div>
              <div className="mt-2.5 pt-2.5 border-t border-stone-200 flex items-center justify-between">
                ...
                >
                  <Facebook className="w-3 h-3" /> {lang === 'en' ? 'Visit Page' : 'Ghé thăm trang'}
                </a>
                <span className="seal-stamp text-[8px] py-0.5 px-1.5">
                  {lang === 'en' ? 'OFFICIAL' : 'CHÍNH HÃNG'}
                </span>
              </div>
            </div>
            ...
            <div>
              <h4 className="text-xs font-serif font-bold text-amber-300 uppercase tracking-widest mb-2.5">
                {lang === 'en' ? 'Social & Reviews' : 'Mạng xã hội & Đánh giá'}
              </h4>
              <div className="flex items-center gap-2.5">
                <a
                  href="https://facebook.com"
        ...
        <div className="pt-8 text-center text-xs text-stone-500 space-y-1.5 font-literary">
          <p className="font-serif font-bold text-amber-400 tracking-wider">
            {lang === 'en' ? 'PHỞ NGỌC HÂN — TEACHER HAN AT CIVIL ENGINEERING UNIVERSITY' : 'QUÁN PHỞ NGỌC HÂN — CÔ GIÁO ĐẠI HỌC XÂY DỰNG'}
          </p>
          <p className="text-stone-400">
            {lang === 'en'
              ? 'Address: 55 Giai Phong, Hai Ba Trung District, Hanoi • Hotline: 0988 567 899'
