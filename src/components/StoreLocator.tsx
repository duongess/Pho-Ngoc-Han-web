...
export const StoreLocator: React.FC = () => {
  const { lang, t, getStoreName, getStoreAddress, getStoreHours } = useLanguage();
  ...
  const filteredBranches = STORE_BRANCHES.filter((branch) => {
    ...
    const name = getStoreName(branch).toLowerCase();
    const address = getStoreAddress(branch).toLowerCase();
    const query = searchQuery.toLowerCase();
    return matchesDistrict && (name.includes(query) || address.includes(query) || branch.name.toLowerCase().includes(query) || branch.address.toLowerCase().includes(query));
  });
  ...
  const handleFindNearest = () => {
    ...
    setLocationToast(
      lang === 'en'
        ? 'Located successfully! Nearest location: Phở Ngọc Hân - Gate of NUCE (55 Giai Phong)'
        : 'Đã định vị thành công! Chi nhánh gần bạn nhất: Phở Ngọc Hân - Cổng ĐH Xây Dựng (55 Giải Phóng)'
    );
    setTimeout(() => {
        ...
        >
          ...
          <div className="bg-gradient-to-r from-[#7f1d1d] via-[#991b1b] to-[#6b1414] text-amber-100 px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-md border-b-2 border-amber-400/40">
            <div>
              <div className="flex items-center gap-2">
                <span className="seal-stamp text-[9px] py-0.5 px-1.5">{t('locator.seal')}</span>
            ...
            >
              ...
              <span>{t('locator.find_nearest')}</span>
            </motion.button>
          </div>
          ...
          <div className="bg-[#fbf7f0] px-6 py-3.5 border-b border-amber-200/80 flex flex-wrap items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-1.5">
              {districts.map((d) => {
                ...
                const districtLabel = d === 'all' ? t('locator.all_districts') : d;
                return (
                  <button
                    key={d}
                  ...
                  >
                    ...
                    <span className="relative z-10">{districtLabel}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="divide-y divide-amber-200/60 max-h-[460px] overflow-y-auto bg-[#fffdfa]">
            {filteredBranches.map((branch) => {
              ...
              const branchName = getStoreName(branch);
              const branchAddress = getStoreAddress(branch);
              const branchHours = getStoreHours(branch);
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                    ...
                    >
                      ...
                      <span>{t('locator.directions')}</span>
                    </a>
                  </div>
                  ...
                  <div className="mt-2.5 space-y-1.5 text-xs sm:text-sm text-stone-700 font-literary">
                    ...
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-1.5 pt-1.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{t('locator.hotline')} <strong className="text-stone-900 font-serif">{branch.hotline}</strong></span>
                      </div>
                      ...
                      {branchHours && (
                        <div className="flex items-center gap-2 text-emerald-800 font-medium">
                          ...
                          <span>{t('locator.hours')} {branchHours}</span>
                        </div>
                      )}
                    </div>
                  </div>
